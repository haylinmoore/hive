...
class MeshCore:
    ...
    async def connect(self):
        ...
        # Try a ping first to ensure communication is working
        try:
            ping_result = await self.commands.send_ping()
            logger.debug(f"Initial ping successful: {ping_result}")
        except Exception as e:
            logger.warning(f"Initial ping failed: {e}, continuing anyway")
        
        # Try to send appstart with retries
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Failed to send appstart (attempt {attempt + 1}/{max_retries}): {e}")
                    # Try to recover the connection
                    if hasattr(self.connection_manager.connection, 'recover_from_error'):
                        await self.connection_manager.connection.recover_from_error()
                    await asyncio.sleep(0.5)
                else:
                    raise ConnectionError(f"Failed to initialize device after {max_retries} attempts: {e}")
