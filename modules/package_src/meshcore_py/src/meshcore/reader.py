...
class MessageReader:
    ...
    async def handle_rx(self, data: bytearray):
        ...
        if not data or len(data) == 0:
            logger.warning("Received empty data packet, ignoring")
            return
        ...
        if packet_type_value == PacketType.OK.value:
            ...
            if len(data) < 148:
                logger.error(f"CONTACT packet too short: {len(data)} bytes")
                return
            ...
            if 36 + plen > len(data):
                logger.error(f"CONTACT path length {plen} exceeds packet size")
                plen = min(plen, len(data) - 36)
            ...
            if len(data) < 58:
                logger.error(f"SELF_INFO packet too short: {len(data)} bytes")
                return
