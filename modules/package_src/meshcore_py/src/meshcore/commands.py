...
class CommandHandler:
    ...
    async def send_ping(self) -> Event:
        """Send a simple time query as a ping to check connectivity"""
        logger.debug("Sending ping (time query)")
        return await self.send(b"\x0B", [EventType.CURRENT_TIME], timeout=2.0)
